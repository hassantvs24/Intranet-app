...
@section('admin-content')
    <div class="container-fluid p-4">
        <div class="row page-header mb-3">
            <div class="col">
                <h3 class="page-title">{{ __('Archived Groups') }}</h3>
            </div>
        </div>

        <div class="row  mb-4 align-items-center align-content-center">
            <div class="col">
                <form action="{{ route('search-group-archive', [ app()->getLocale() ] ) }}" method="get">
                    ...
                    <div class="form-row">
                        <div class="col col-md-5">
                            <div class="input-group">
                                ...
                                <input type="search" class="form-control border-left-0" id="inputGroupFile01" aria-describedby="search-icon"
                                name="search" placeholder="{{ __('Search') }}" value="@if( !empty( $_GET['search'] ) ) {{ $_GET['search'] }} @endif ">
                            </div>
                        </div>
                    </div>
                </form>
        ...
        <div class="row">
            <div class="col">
                <a href="{{ route('all-groups', app()->getLocale()) }}" class="btn btn-outline-primary">{{ __('view Active Groups') }}</a>
            </div>
        </div>
    </div>
@endsection
